 ...
 * auto-injects ``props.close``). On confirm, calls ``props.onConfirm(tagIds, mode)``
 * with the full list of tag ids and the mode ("add" or "remove").
 */
export class LLMBulkTagDialog extends Component {
    ...
    async _loadTags() {
        try {
            const tags = await this.orm.searchRead("llm.thread.tag", [], ["id", "name", "color"], {
                order: "name",
            });
    ...
    toggleMode() {
        this.state.mode = this.state.mode === "add" ? "remove" : "add";
    }
    ...
    async _createNewTag() {
        ...
        try {
            ...
            const [tagId] = await this.orm.call("llm.thread.tag", "name_create", [name]);
    ...
    async onConfirm() {
        ...
        await this.props.onConfirm?.(ids, this.state.mode);
        this.props.close?.();
    }
